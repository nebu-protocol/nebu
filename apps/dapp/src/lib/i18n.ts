...
export const EN_DICT: Record<string, string> = {
  ...
  "Nebu menyediakan automated liquidity provision di PancakeSwap Infinity (BNB Smart Chain). Semua angka APR/PnL bersifat gross (pre-IL), berbasis simulasi, dan bukan jaminan hasil. Menyediakan likuiditas pada pool volatil (mis. memecoin) berisiko tinggi termasuk impermanent loss dan kehilangan sebagian/seluruh modal. Tidak ada di halaman ini yang merupakan nasihat investasi, hukum, pajak, atau finansial.":
    "Nebu provides automated liquidity provision on PancakeSwap Infinity (BNB Smart Chain). All APR/PnL figures are gross (pre-IL), simulation-based, and not a guarantee of returns. Providing liquidity to volatile pools (e.g. memecoins) is high-risk, including impermanent loss and the loss of part or all of your capital. Nothing on this page constitutes investment, legal, tax, or financial advice.",
  "Nebu tidak mengumpulkan data pribadi. Yang kami simpan hanya data on-chain publik (pool, posisi) dan—bila kamu mengaktifkan automation—private key wallet yang kamu berikan, disimpan terenkripsi (AES-256-GCM) di server dan hanya dipakai untuk menjalankan strategi LP. Connect wallet hanya membaca alamat publik.":
    "Nebu collects no personal data. We store only public on-chain data (pools, positions) and—if you enable automation—the wallet private key you provide, kept encrypted (AES-256-GCM) on the server and used only to run the LP strategy. Connecting a wallet only reads your public address.",
  "Nebu disediakan apa adanya, tanpa jaminan. Menyediakan likuiditas berisiko (impermanent loss, rug, kehilangan modal). Angka APR/PnL bersifat gross dan simulasi—bukan jaminan hasil dan bukan nasihat finansial. Dengan menggunakan layanan ini kamu menerima seluruh risiko atas dana yang kamu kelola.":
    "Nebu is provided as-is, without warranty. Providing liquidity is risky (impermanent loss, rugs, loss of capital). APR/PnL figures are gross and simulation-based—not a guarantee of returns and not financial advice. By using this service you accept all risk over the funds you manage.",
};
