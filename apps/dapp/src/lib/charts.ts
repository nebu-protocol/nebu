 ...
 * events in demo mode).
 */
export function fundingProgressSeries(
  listMs: number,
  fills: readonly { timestamp: number; cumulativePct: number }[],
): SeriesPoint[] {
  return [
...
/** One bond position feeding the portfolio value curve. */
export type HoldingInput = {
  /** What the wallet paid for the position (USDC). */
  costUsdc: number;
  /** What the position redeems at maturity (USDC). */
  faceUsdc: number;
  /** First fill — the position contributes 0 before this. */
  purchaseMs: number;
  listMs: number;
  maturityMs: number;
};
